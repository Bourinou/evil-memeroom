...
export async function scanClam(file, { host = process.env.CLAMAV_HOST, port = Number(process.env.CLAMAV_PORT || 3310), timeoutMs = 5 * 60 * 1000 } = {}) {
  if (!host) throw mediaError('Analyse antivirus indisponible. Contactez le gestionnaire du serveur.', 503);
  ...
  const result = new Promise((resolve, reject) => {
    ...
    socket.on('end', () => { if (!reply.includes('\0')) reject(new Error('Incomplete antivirus response')); });
  });
  ...
  try {
    ...
    if (response.endsWith(' FOUND')) throw mediaError('Fichier refusé par l’antivirus (menace détectée ou limites d’analyse dépassées).', 422);
    if (response !== 'stream: OK') throw new Error('Antivirus did not return OK');
  } catch (error) {
    if (error.status) throw error;
    throw mediaError('Analyse antivirus indisponible ou incomplète. Le fichier n’a pas été partagé.', 503);
  } finally { clearTimeout(timer); input.destroy(); socket.destroy(); }
}

export async function inspectMedia(asset) {
  if (process.env.CLAMAV_HOST || process.env.MEMEROOM_SCAN_REQUIRED === '1') await scanClam(asset.file);
