...
export class PasswordLimiter {
  ...
  consume(ip, code) {
    ...
    for (const [key, maximum] of keys) {
      ...
      if (item && item.until > now && item.count >= maximum)
        throw Object.assign(
          new Error(
            `Trop de mots de passe incorrects. Réessayez dans ${Math.ceil((item.until - now) / 1000)} s.`,
          ),
    ...
    for (const [key] of keys) {
      ...
      if (!item || item.until <= now) {
        item = { count: 0, until: now + 5 * 60 * 1000 };
        this.budgets.set(key, item);
      }
      item.count++;
    }
    return () => {
  ...
  sweep() {
    for (const [key, item] of this.budgets) if (item.until <= this.now()) this.budgets.delete(key);
  }
}
