...
if (
  !['list', 'delete', 'status'].includes(action) ||
  (action === 'delete' && !/^[A-Z2-9]{8}$/.test(code))
) {
  throw new Error('Utilisation : npm run rooms -- list | status | delete CODE');
}
...
const endpoint = action === 'status' ? 'status' : `rooms${action === 'delete' ? '/' + code : ''}`;
const response = await fetch(`${server}/api/admin/${endpoint}`, {
