...
const binaryPattern = new RegExp(
  `^MemeRoom-(?:Setup-${versionPattern}\\.exe(?:\\.blockmap)?|${versionPattern}-Linux-x86_64\\.AppImage|${versionPattern}-Mac-(?:arm64|x64)\\.zip)$`,
);
export const releaseFilename = (name) =>
  typeof name === 'string' &&
  (binaryPattern.test(name) || ['latest.yml', 'latest-linux.yml'].includes(name));
...
export async function listDownloads(directory) {
  ...
  let manifest;
  try {
    manifest = JSON.parse(await readFile(await containedFile(directory, 'downloads.json'), 'utf8'));
  } catch {
    return result;
  }
    ...
    try {
      ...
      if (info.isFile() && info.size > 0)
        result[platform] = {
          ...
          bytes: info.size,
          url: `/releases/${expected}`,
        };
    } catch {
      /* An unpublished platform has no active download button. */
    }
  }
  return result;
}
...
export async function serveRelease(req, res, directory, name) {
  ...
  let handle;
  try {
    handle = await open(await containedFile(directory, name), 'r');
  } catch {
    res.writeHead(404);
    res.end();
    return;
  ...
  try {
    ...
    res.setHeader(
      'Content-Type',
      metadata ? 'text/yaml; charset=utf-8' : 'application/octet-stream',
    );
