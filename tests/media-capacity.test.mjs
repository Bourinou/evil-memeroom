...
test('room and global byte limits evict as many oldest uploads as needed', () => {
  const a = { code: 'a', bytes: 6, media: new Map() },
    b = { code: 'b', bytes: 6, media: new Map() },
    c = { code: 'c', bytes: 0, media: new Map() };
  const rooms = new Map([
      ['a', a],
      ['b', b],
      ['c', c],
  ...
  for (const [id, room] of [
    ...
    const item = { id, bytes: 3, roomCode: room.code };
    room.media.set(id, item);
    media.set(id, item);
  }
  const limits = { mediaCount: 30, roomBytes: 10, totalBytes: 12 };
  const space = makeMediaSpace(c, rooms, media, 5, 12, limits);
  assert.deepEqual([...media.keys()], ['a2', 'b2']);
  assert.equal(space.totalBytes, 6);
  assert.equal(a.bytes, 3);
  assert.equal(b.bytes, 3);
  assert.deepEqual([...space.changed], [a, b]);
  const local = makeMediaSpace(a, rooms, media, 8, space.totalBytes, limits);
  assert.equal(a.media.size, 0);
  assert.equal(a.bytes, 0);
  assert.equal(local.totalBytes, 3);
});
...
test('the 31st valid upload replaces the oldest; an invalid upload never deletes anything', async (t) => {
  ...
  ws.send(JSON.stringify({ id: 'create', type: 'create', data: { name: 'Test' } }));
