...
test('password budgets survive reconnects, isolate addresses, expire, and ignore untrusted proxy headers', () => {
  let now = 0;
  const limit = new PasswordLimiter({ now: () => now });
  for (let i = 0; i < 5; i++) limit.consume('a', 'room');
  assert.throws(() => limit.consume('a', 'room'), /300 s/);
  limit.consume('b', 'room')();
  now = 300001;
  limit.sweep();
  limit.consume('a', 'room')();
  const request = {
    socket: { remoteAddress: '127.0.0.1' },
    headers: { 'x-forwarded-for': '1.1.1.1, 2.2.2.2' },
  };
  assert.equal(clientIP(request), '127.0.0.1');
  assert.equal(clientIP(request, true), '2.2.2.2');
  assert.equal(protocol.cleanSettings({ receiveOwn: false }).receiveOwn, undefined);
  assert.equal(protocol.cleanSettings().size, 60);
...
test('antivirus protocol accepts only OK, rejects detections and unavailable engines', async (t) => {
  ...
  t.after(() => new Promise((r) => daemon.close(r)));
  ...
  answer = 'stream: Test-Signature FOUND\0';
  await assert.rejects(scanClam(file, options), { status: 422 });
  answer = 'stream: scan failed ERROR\0';
  await assert.rejects(scanClam(file, options), { status: 503 });
});
...
test('expired media is deleted from disk while members stay connected; saved messages reupload it', async (t) => {
  ...
  const server = createRoomServer({
    ...
    dataDir: null,
    mediaTtlMs: 180,
    scanMedia: async (asset) => {
      files.push(asset.file);
      if (rejectScan) throw Object.assign(new Error('Antivirus indisponible'), { status: 503 });
    },
  });
  ...
  ws.send(JSON.stringify({ id: '1', type: 'create', data: { name: 'Test' } }));
  const room = JSON.parse((await reply)[0]).data;
  const png = await readFile('public/icon.png');
  const upload = () =>
    fetch(base + '/api/media', {
      method: 'POST',
      headers: { Authorization: `Bearer ${room.token}`, 'X-Filename': 'icon.png' },
      body: png,
    });
  ...
  const saved = await presets.save({
    name: 'Réaction favorite',
    reaction: { server: base, media: asset, caption: 'Salut', duration: 2 },
  });
  await new Promise((r) => setTimeout(r, 420));
  assert.equal((await fetch(base + asset.url)).status, 404);
  await assert.rejects(access(files[0]));
  assert.equal(ws.readyState, WebSocket.OPEN);
  const reopened = new Presets(directory, protocol);
  assert.equal((await reopened.list())[0].name, 'Réaction favorite');
  const loaded = await reopened.load(saved.id, base, room.token);
  assert.equal(loaded.caption, 'Salut');
  assert.notEqual(loaded.media.id, asset.id);
  assert.equal((await fetch(base + loaded.media.url)).status, 200);
  rejectScan = true;
  assert.equal((await upload()).status, 503);
  await new Promise((r) => setTimeout(r, 20));
  await assert.rejects(access(files.at(-1)));
  await reopened.rename(saved.id, 'Encore');
  assert.equal((await reopened.list())[0].name, 'Encore');
