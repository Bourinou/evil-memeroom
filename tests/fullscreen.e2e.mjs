...
const env = {
  ...process.env,
  MEMEROOM_USER_DATA: path.resolve('.test-artifacts', `fullscreen-${Date.now()}`),
...
try {
  ...
  let control;
  for (let i = 0; i < 100; i++) {
    control = app.windows().find((w) => w.url() === 'http://localhost/remote');
    if (control) break;
    await new Promise((r) => setTimeout(r, 50));
  }
  assert.ok(control);
  await control.waitForFunction(() => document.body.dataset.ready === 'true');
  await app.evaluate(async ({ BrowserWindow }) => {
    const target = new BrowserWindow({
      title: 'MemeRoom fullscreen test',
      fullscreen: true,
      autoHideMenuBar: true,
      webPreferences: { sandbox: true, contextIsolation: true, nodeIntegration: false },
    });
    await target.loadURL(
      'data:text/html,<title>MemeRoom fullscreen test</title><body style="background:%23223448;color:white"><h1>Test plein écran MemeRoom</h1><input autofocus placeholder="Application sous l’overlay">',
    );
  ...
  await control.evaluate(async () => {
    ...
    await window.memeroom.show({
      id: 'fullscreen',
      server: info.server,
      duration: 15,
      caption: 'PAR-DESSUS LE PLEIN ÉCRAN',
      sender: { name: 'Rose' },
    });
  });
  ...
  const handles = await app.evaluate(({ BrowserWindow }) => {
    const target = BrowserWindow.getAllWindows().find(
      (w) => w.getTitle() === 'MemeRoom fullscreen test',
    );
    const overlay = BrowserWindow.getAllWindows().find((w) => w.getTitle() === 'MemeRoom Overlay');
    target.moveTop();
    target.focus();
    return {
      target: target.getNativeWindowHandle().readBigUInt64LE().toString(),
      overlay: overlay.getNativeWindowHandle().readBigUInt64LE().toString(),
  ...
  const state = JSON.parse(
    execFileSync(
      ...
      [
        ...
        '-NonInteractive',
        '-File',
        path.resolve('tests/window-order.ps1'),
        '-OverlayHandle',
        handles.overlay,
        '-TargetHandle',
        handles.target,
  ...
  const focus = await app.evaluate(({ BrowserWindow }) => ({
    game: BrowserWindow.getAllWindows()
      .find((w) => w.getTitle() === 'MemeRoom fullscreen test')
      .isFocused(),
    overlay: BrowserWindow.getAllWindows()
      .find((w) => w.getTitle() === 'MemeRoom Overlay')
      .isFocused(),
  }));
  assert.deepEqual(
    focus,
    { game: true, overlay: false },
    'Fullscreen test window retains Electron focus',
  );
  ...
  console.log(
    'OK · Plein écran Windows : overlay au-dessus après reprise du premier plan, focus conservé et passage des clics configuré.',
  );
} finally {
  await app.close();
}
