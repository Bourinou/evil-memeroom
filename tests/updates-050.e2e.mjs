...
const server=http.createServer((_req,res)=>{res.writeHead(200,{'Content-Type':'image/png','Content-Length':png.length});res.write(png.subarray(0,12));setTimeout(()=>res.end(png.subarray(12)),2500);});
await new Promise(r=>server.listen(0,'127.0.0.1',r));
const profile=path.resolve('.test-artifacts',`updates-050-${Date.now()}`);
const env={...process.env,MEMEROOM_USER_DATA:profile,MEMEROOM_ALLOW_MULTIPLE:'1',MEMEROOM_PORT:'0',HOST:'127.0.0.1',MEMEROOM_DISABLE_UPDATES:'1'};delete env.ELECTRON_RUN_AS_NODE;
let app;const errors=[];
try {
  app=await electron.launch({args:['.'],env,chromiumSandbox:true});await app.firstWindow();
  let page;
  ...
  assert.ok(page);page.on('pageerror',e=>errors.push(e.message));await page.waitForFunction(()=>document.body.dataset.ready==='true');await page.bringToFront();
  assert.equal(await page.locator('#setting-own').count(),0);
  await page.click('#open-settings');assert.equal(await page.inputValue('#setting-size'),'60');
  await page.screenshot({path:'.test-artifacts/settings-050.png'});
  await page.mouse.click(8,8);await page.waitForFunction(()=>!document.querySelector('#settings-dialog').open);
  await page.fill('#caption','QUAND ÇA MARCHE');await page.click('#save-preset');await page.fill('#preset-name','Victoire');await page.click('#preset-submit');
  await page.waitForFunction(()=>!document.querySelector('#preset-dialog').open);await page.click('#show-presets');await page.getByText('Victoire',{exact:true}).waitFor();
  ...
  await new Promise(r=>setTimeout(r,1100));assert.equal(await overlay.locator('.reaction-view').count(),1);
  await overlay.waitForFunction(()=>!document.querySelector('.reaction-view'));
  assert.deepEqual(errors,[]);
  console.log('OK · Réglages fermés par clic extérieur, taille maximale, messages enregistrés, texte mème et compteur après téléchargement lent.');
