...
try {
  ...
  await app.evaluate(({ globalShortcut }) => {
    ...
    globalShortcut.register = (value, callback) => {
      const result = register(value, callback);
      if (result) globalThis.shortcutCallbacks.set(value, callback);
      return result;
    };
  });
  await page.bringToFront();
  await page.click('#open-settings');
  await page.click('#dismiss-shortcut');
  await page.getByText('Appuyez sur les touches…', { exact: true }).waitFor();
  await page.keyboard.press('Control+Alt+F10');
  await page.waitForFunction(() =>
    document.querySelector('#shortcut-hint').textContent.startsWith('Raccourci enregistré'),
  );
  assert.equal(
    await app.evaluate(({ globalShortcut }) => globalShortcut.isRegistered('Control+Alt+F10')),
    true,
  );
  assert.equal(
    JSON.parse(await readFile(path.join(profile, 'preferences.json'), 'utf8')).dismissShortcut,
    'Control+Alt+F10',
  );
  await page.evaluate(async (server) => {
    ...
    await window.memeroom.show({
      ...
      caption: 'À couper',
      duration: 15,
      audio: { id, kind: 'audio', mime: 'audio/wav', name: 'test.wav', url: '/media/' + id },
      sender: { name: 'Test' },
    });
  ...
  assert.equal(
    await page.evaluate(async () => (await window.memeroom.info()).settings.paused),
    false,
  );
  ...
  await assert.rejects(
    page.evaluate(() => window.memeroom.saveDismissShortcut('Control+Alt+F11')),
    /utilisé|indisponible/,
  );
  assert.equal(
    await page.evaluate(async () => (await window.memeroom.info()).settings.dismissShortcut),
    'Control+Alt+F10',
  );
  ...
  console.log(
    'OK · Raccourci personnalisé enregistré ; image et audio interrompus sans pause ; conflits et désactivation pris en charge.',
  );
} finally {
  await app?.close();
