...
app.whenReady().then(() => {
  global.qaBrowser = new BrowserWindow({
    show: false,
    width: 920,
    height: 740,
...
global.runDistributionCheck = async ({ url, directory, platform, version = '0.2.9' }) => {
  ...
  const adapter = {
    version,
    name: 'memeroom-distribution-test',
    isPackaged: true,
    appUpdateConfigPath,
    ...
    onQuit() {},
    quit() {
      throw new Error('Tests must not run the installer');
    },
  };
...
global.beginStartupWindowCheck = () => {
  ...
  const originalCheck = autoUpdater.checkForUpdates,
    originalDownload = autoUpdater.downloadUpdate;
  global.startupOutcome = null;
  global.startupDownloads = 0;
  autoUpdater.checkForUpdates = () =>
    new Promise((resolve) => {
      global.finishStartupCheck = resolve;
    });
  autoUpdater.downloadUpdate = async () => {
    global.startupDownloads++;
  };
  const facade = {
    isPackaged: true,
  ...
  checkStartupUpdate({ app: facade, BrowserWindow, ipcMain }).then((result) => {
    ...
    autoUpdater.downloadUpdate = originalDownload;
  });
};
