...
window.overlay.onShow(event => { clear(); current = mountReaction(root, event, { volume: event.volume, autoplay: true, onReady: () => window.overlay.ready(event.playbackId), onDone: () => window.overlay.done(event.playbackId), onProgress: () => window.overlay.progress(event.playbackId), onError: error => { console.error('Lecture overlay :', error?.message); window.overlay.error('Impossible de lire ce média. Vérifiez le fichier ou sa connexion au serveur.', event.playbackId); } }); });
