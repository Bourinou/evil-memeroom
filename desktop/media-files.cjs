...
function assetURL(asset, server) {
  ...
  if (!/^[A-Za-z0-9_-]{32}$/.test(asset?.id) || asset.url !== `/media/${asset.id}` || !['image','video','audio'].includes(asset.kind)) throw new Error('Média invalide.');
...
async function downloadAsset(asset, server, destination, signal) {
  ...
  const response = await fetch(assetURL(asset, server), { redirect:'error', signal:signal ? AbortSignal.any([signal, timedSignal]) : timedSignal });
  if (!response.ok) throw new Error('Fichier indisponible ou expiré. Importez-le à nouveau.');
  if (Number(response.headers.get('content-length')) > MAX_BYTES) { await response.body.cancel(); throw new Error('Le fichier dépasse 1 Go.'); }
  let bytes = 0;
  const limit = new Transform({ transform(chunk, _encoding, callback) {
    bytes += chunk.length; callback(bytes > MAX_BYTES ? new Error('Le fichier dépasse 1 Go.') : null, chunk);
  } });
  try {
    await pipeline(response.body, limit, createWriteStream(destination, { flags:'wx', mode:0o600 }), { signal:signal ? AbortSignal.any([signal, timedSignal]) : timedSignal });
...
async function uploadAsset(file, asset, server, token) {
  const base = new URL(server);
  if (!['http:','https:'].includes(base.protocol) || base.username || base.password || base.pathname !== '/' || base.search || base.hash || !/^[A-Za-z0-9_-]{32}$/.test(token)) throw new Error('Connexion invalide.');
  const stream = createReadStream(file);
  try {
    const response = await fetch(new URL('/api/media', base), { method:'POST', redirect:'error', headers:{ Authorization:`Bearer ${token}`, 'Content-Type':'application/octet-stream', 'Content-Length':String(asset.bytes), 'X-Filename':encodeURIComponent(asset.name) }, body:stream, duplex:'half', signal:AbortSignal.timeout(TRANSFER_MS) });
