...
async function checkStartupUpdate({ app, BrowserWindow, ipcMain }) {
  const disabled =
    ...
    /** @type {{memeroomUpdatesEnabled?: boolean}} */ (require('../package.json'))
      .memeroomUpdatesEnabled === false;
  ...
  try {
    ...
    ipcMain.removeListener('update:skip', skip);
  }
}
