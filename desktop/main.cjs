const { app, BrowserWindow, ipcMain, screen, Tray, Menu, nativeImage, globalShortcut, session, dialog } = require('electron');
...
if (process.env.MEMEROOM_USER_DATA) app.setPath('userData', path.resolve(process.env.MEMEROOM_USER_DATA));
...
function clearOverlay() {
  generation++; clearTimeout(hideTimer);
...
function updateTray() {
  if (!tray) return;
  tray.setToolTip(settings.paused ? 'MemeRoom · réception en pause' : 'MemeRoom · réception active');
  tray.setContextMenu(Menu.buildFromTemplate([
    { label: 'Ouvrir MemeRoom', click: () => { control.show(); control.focus(); } },
    { label: settings.paused ? 'Reprendre la réception' : 'Mettre en pause', click: () => saveSettings({ ...settings, paused: !settings.paused }).catch(() => {}) },
    { type: 'separator' }, { label: 'Quitter MemeRoom', click: () => { quitting = true; app.quit(); } }
  ]));
}
...
if (singleInstance) app.whenReady().then(async () => {
  ...
  control = new BrowserWindow({ title: 'MemeRoom', width: 860, height: 780, minWidth: 540, minHeight: 620, backgroundColor: '#f7f7f8', autoHideMenuBar: true, icon, webPreferences: { session: controlSession, preload: path.join(__dirname, 'preload.cjs'), sandbox: true, contextIsolation: true, nodeIntegration: false, backgroundThrottling: false } });
  overlay = new BrowserWindow({ title: 'MemeRoom Overlay', width: 520, height: 420, show: false, frame: false, transparent: true, backgroundColor: '#00000000', hasShadow: false, alwaysOnTop: true, skipTaskbar: true, focusable: false, resizable: false, movable: false, minimizable: false, maximizable: false, fullscreenable: false, webPreferences: { preload: path.join(__dirname, 'overlay-preload.cjs'), sandbox: true, contextIsolation: true, nodeIntegration: false, backgroundThrottling: false, autoplayPolicy: 'no-user-gesture-required' } });
  ...
  ipcMain.handle('overlay:test', event => { if (!trustedControl(event)) throw new Error('Accès refusé.'); return displayReaction({ server: baseUrl, duration: 4, caption: 'Test de l’overlay', sender: { name: clientState?.nickname || 'MemeRoom' } }, true); });
  ...
  startupUpdate.close();
}).catch(error => { dialog.showErrorBox('MemeRoom', `Impossible de démarrer : ${error.message}`); quitting = true; app.quit(); });
