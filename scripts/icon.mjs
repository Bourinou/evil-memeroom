  ...
  for (let y = 0; y < size; y++)
    for (let x = 0; x < size; x++) {
      ...
      const offset = y * (size * 4 + 1) + 1 + x * 4,
        cornerX = Math.max(11 - ux, 0, ux - 52),
        cornerY = Math.max(11 - uy, 0, uy - 52);
      const opaque = cornerX * cornerX + cornerY * cornerY < 144;
      const px = Math.floor((ux - 6) / 4),
        py = Math.floor((uy - 14) / 4),
        mark = pattern[py]?.[px] === '1';
