...
for (const arch of ['x64', 'arm64']) {
  ...
  const writing = pipeline(
    zip.outputStream,
    createWriteStream(path.join(directory, `MemeRoom-${version}-Mac-${arch}.zip`)),
  );
  zip.end();
  await writing;
