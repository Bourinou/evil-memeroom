...
export async function stageReleases({ version, windowsDir, linuxDir, macDir, outputDir }) {
  ...
  for (const [platform, directory, metadata, filename] of [
    ...
    if (manifest?.version !== version || manifest?.path !== filename || manifest?.files?.length !== 1 || !entry) throw new Error(`${metadata} ne correspond pas à la version ${version}.`);
    const digest = await sha512(source);
    const size = (await stat(source)).size;
    if (digest !== entry.sha512 || digest !== manifest.sha512 || size !== entry.size) throw new Error(`Fichier modifié ou incomplet : ${filename}`);
    const binaries = [filename];
    if (platform === 'windows') { await stat(`${source}.blockmap`); binaries.push(`${filename}.blockmap`); }
  ...
  if (macDir) {
    ...
    for (const platform of ['macArm64', 'macIntel']) {
      const filename = downloadFilenames(version)[platform], item = metadata[platform];
      const source = path.join(macDir, filename);
      if (item?.version !== version || item?.filename !== filename || item?.sha512 !== await sha512(source) || item?.bytes !== (await stat(source)).size) throw new Error(`Archive Mac modifiée ou incomplète : ${filename}`);
      binaries.push(filename); downloads[platform] = { version, filename };
    }
    plans.push({ directory: macDir, binaries });
  }
  // Validate both builds before changing the directory served to clients.
  await mkdir(outputDir, { recursive: true });
  for (const plan of plans) for (const name of plan.binaries) {
    const target = path.join(outputDir, name), source = path.join(plan.directory, name);
    let existing;
    try { existing = await sha512(target); } catch (error) { if (error.code !== 'ENOENT') throw error; }
    if (existing && existing !== await sha512(source)) throw new Error(`La version publiée est immuable : ${name}. Incrémentez la version.`);
  }
  for (const plan of plans) for (const name of plan.binaries) {
  ...
  await writeFile(path.join(outputDir, 'downloads.json.tmp'), JSON.stringify(downloads, null, 2) + '\n');
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  const result = await stageReleases({ version, windowsDir: path.resolve(process.argv[2] || path.join(root, `release/build-${version}`)), linuxDir: path.resolve(process.argv[3] || path.join(root, 'release')), macDir: path.resolve(process.argv[4] || path.join(root, 'release/mac')), outputDir: path.join(root, 'releases') });
