...
export function mountReaction(
  ...
  const sender = cleanText(
    typeof reaction.sender === 'string' ? reaction.sender : reaction.sender?.name,
    24,
  );
  if (sender) {
    const label = document.createElement('div');
    label.className = 'reaction-sender';
  ...
  if (reaction.media) {
    ...
    if (reaction.media.kind === 'video') {
      ...
      visual.muted = visual === mutedVideo || volume === 0;
      visual.volume = volume / 100;
      players.push(visual);
    } else visual.alt = reaction.media.name;
    visuals.push([visual, reaction.media]);
    art.append(visual);
    root.append(art);
  }
  const caption = document.createElement('div');
  caption.className = 'reaction-caption';
  caption.textContent = reaction.caption || '';
  caption.hidden = !reaction.caption;
  root.append(caption);
  if (reaction.audio && !still) {
    ...
    players.push(audio);
    visuals.push([audio, reaction.audio]);
    root.append(audio);
  }
  ...
  async function prepare(element, asset) {
    ...
    await new Promise((resolve, reject) => {
      ...
      const failed = () => {
        cleanup();
        reject(new Error('Fichier image, vidéo ou audio illisible.'));
      };
      const cleanup = () => {
      ...
      element.addEventListener(
        element.tagName === 'IMG' ? 'load' : still ? 'loadedmetadata' : 'loadeddata',
        loaded,
        { once: true },
      );
  ...
  async function start() {
    ...
    if (automatic) {
      const progress = new Map(
        players.map((player) => [player, { time: player.currentTime, at: performance.now() }]),
      );
      const checkEnded = () => {
        if (players.every((player) => player.ended)) finish();
      };
      ...
      watchdog = setInterval(() => {
        ...
        for (const player of players) {
          ...
          if (player.currentTime !== previous.time) {
            previous.time = player.currentTime;
            previous.at = now;
            advanced = true;
          } else if (now - previous.at >= LIMITS.playbackStallMs) {
            fail(new Error('La lecture du média est bloquée.'));
            return;
          }
        }
        if (advanced) onProgress();
        checkEnded();
      }, 1000);
      checkEnded();
    ...
    const tick = () => {
      if (dead || completed) return;
      const elapsed = players.length
        ? Math.max(...players.map((player) => player.currentTime))
        : (performance.now() - startedAt) / 1000;
      const cue = reaction.subtitles?.find((c) => elapsed >= c.start && elapsed < c.end);
      caption.textContent = cue?.text || reaction.caption || '';
  ...
  return {
    setVolume(value) {
      ...
      for (const player of players) {
        ...
        player.muted = player === mutedVideo || value === 0;
      }
    },
    destroy() {
      dead = true;
